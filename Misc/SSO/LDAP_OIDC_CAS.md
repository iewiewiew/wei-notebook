...
核心概念：什么是 SSO 和 CAS？  
SSO：单点登录。它是一种身份验证方案，允许用户使用一组凭证登录多个相关的、但相互独立的软件系统。用户只需登录一次，就可以访问所有被授权的系统，无需重复输入用户名和密码。  
CAS：中央认证服务（ Central Authentication Service ） 。它是一种实现 SSO 的具体协议和软件框架。最初由耶鲁大学开发，现在是 Apereo 基金会 旗下的一个开源项目。CAS 提供了完整的企业级单点登录解决方案。  
简单来说：SSO 是目标（做什么），CAS 是实现这个目标的一种流行且标准化的方法（怎么做）。  
通常指的是基于 CAS 协议实现的单点登录系统。