...
- 切换两个 Tab：Control + Tab （注：不是 Command）
- 返回上一个光标：Command + U / Option + Command + ⬅️（左箭头）
- 向下复制当前：Shift + Option + ⬇️（下箭头）
- 运行 Java 程序：Fn + F5
- 向下复制：Shift + Option + ↓
- 格式化文件：Shift + Option + F
- 打开 Terminal：Command + J
- 跳转到指定行：Control + G，输入指定行按回车
...
实现点击 @/utils/xxx.js 点击跳转完整的方案，在项目根目录（和 src 同级）新建 / 修改 jsconfig.json，直接复制配置：
```
{
  "compilerOptions": {
    "baseUrl": ".",
...
问题：Cursor 和 Trae 的文件编辑器，无法按 enter 回车、无法按 x 删除字符  
解决：安全模式启动（排除插件） Cursor：Ctrl+Shift+P → Cursor: Restart with Extensions Disabled  
问题：但是找不到是哪个插件影响  
解决*（危险）：删除 ~/.cursor，这会删除所有 cursor 配置，谨慎执行  

方式1: 永久设置：单击文件直接新开标签（推荐）
VS Code 默认**预览模式**，单击文件会覆盖当前标签；关闭预览即可单击就新开tab。
1. 打开设置
    - Windows/Linux：`Ctrl + ,`
    - Mac：`Cmd + ,`
2. 搜索 `preview`
3. 取消勾选两项：
    ...
    - `Workbench › Editor: Enable Preview From Quick Open`（Ctrl+P搜索文件也不预览）

方式2：编辑 settings.json 配置
