...
安全隔区 GID（硬件）：完全不能刷新 / 修改，出厂即固定
用户组 GID（系统账户）：可改，但不是设备唯一码
设备唯一 UUID（IOPlatformUUID）：
Intel Mac：重置 NVRAM（Command+Option+P+R）可刷新
Apple Silicon（M 系列）：无法刷新，唯一办法是更换主板



## 临时记录

如何给网站生成一个 License，用于控制网站的有效期？
