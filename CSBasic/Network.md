...
2XX 成功
- 200 OK，表示从客户端发来的请求在服务器端被正确处理（如 GET 或 POST）
- 201 Created：请求成功且服务器创建了新资源（如 POST 创建了新对象）
- 202 Accepted：请求已接受，但尚未处理完成（异步任务）
- 204 No content，表示请求成功，但响应报文不含实体的主体部分（如 DELETE 请求）
...
ping 命令对一个域名进行测试时出现超时，但实际上可以通过其他方式正常访问该域名（例如，使用浏览器），那么可能是因为 ICMP（Internet Control Message Protocol）被阻止了。  
ICMP 流量被阻止或限制的原因有很多。一些网络管理员或互联网服务提供商（ISP）可能出于安全考虑或为了优化网络资源而选择阻止 ICMP 的回显请求。此外，一些防火墙或安全设备可能设置了 ICMP 过滤规则。

Q：Linux 服务器如何查看带宽被打满？  
A：iftop 是一个实时流量监控工具，可以显示网络接口的使用情况。  
apt install iftop   
iftop -i eth0  
