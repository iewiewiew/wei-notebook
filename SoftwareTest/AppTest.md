...
android 工程编译完成会得到我们想要的 apk 安装包，apk 文件其实是一个压缩包，所以可以直接修改后缀名为 zip 或 rar 后能直接解压打开。

1. AndroidManifest.xml：Android 主配置文件，每个应用都必须定义和包含的，它描述了应用的名字、版本、权限、引用的库文件等信息，编译过程中由文本格式转化为二进制AXML文件格式。
2. Assets文件夹：保存一些额外的资源文件，如游戏的声音文件，字体文件等等，在代码中可以用 AssetManager 获取 assets 文件夹的资源。
3. Lib文件夹：存放用 C/C++ 编写的，用 ND K编译生成的 so 文件，供 java 端调用。
...
微信小程序的测试需要覆盖功能、性能、兼容性、安全性和用户体验等多个方面。以下是全面的测试方法：

一、测试环境准备

1. **开发工具测试**：使用微信开发者工具进行初步测试
2. **真机测试**：准备不同品牌、型号、系统的手机
3. **微信版本覆盖**：测试不同微信版本下的兼容性
...
1. 不同网络环境（WiFi/4G/5G/弱网）
2. 网络切换测试（WiFi与移动数据切换）
3. 断网恢复测试
