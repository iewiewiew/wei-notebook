...
- type=index，索引全扫描
- type=range，索引范围扫描
- type=ref，非唯一性索引：使用的索引允许重复的值（这是 ref 与 eq_ref 的主要区别）
- type=eq_ref，唯一索引
- type=NULL，MySQL 不用访问表或者索引，直接就能够得到结果（性能最好）

场景：
MySQL explain SQL 的 rows 值非常大说明什么？
rows 值非常大，表示 MySQL 优化器根据统计信息预估，为了执行该查询（或该步骤），需要检查（读取并扫描）的数据行数非常多。它表示 MySQL 为了找到所需的行，预计需要读取多少行。注意是“读取”，而不是“返回”。

| 情况                                 | 含义             | 行动建议                                                     |
| :----------------------------------- | :--------------- | :----------------------------------------------------------- |
| **`rows` 很大，且 `type=ALL`**       | **全表扫描**     | **必须优化**。检查 `WHERE` 条件，添加索引。                  |
| **`rows` 很大，但 `type=index`**     | **全索引扫描**   | 比全表扫描好，但仍有优化空间。检查是否可使用覆盖索引或更精确的索引。 |
| **`rows` 很大，但 `type=range/ref`** | **索引范围查找** | 查询本身需要处理大量数据。确保这是最优索引，考虑业务上是否真需要这么多数据。 |
| **`rows` 预估不准**                  | **统计信息过期** | 运行 `ANALYZE TABLE` 更新统计信息。                          |

给出一个通过 explain 排查 MySQL 慢查询 SQL 以及优化过程的详细示例？
...
[Docker 安装](../../DevOps/Docker/Docker%20安装软件.md)，配置地址：`/var/lib/mysql`、`/etc/my.cnf`，此处已做目录挂载，可在宿主机修改配置后重启容器：`docker restart mysql`。
...
```
python binlog2sql.py --host=127.0.0.1 --port=3306 --user=root --password=123456 --database=test_platform --start-file='mysql-bin.000001' --flashback > output.sql
```
...
# 筛选耗时超过5秒的SQL（实际上不展示 SQL）
grep "Query_time: [5-9]" /var/lib/mysql/slow.log

# 用官方工具 mysqldumpslow 统计分析
# 按平均耗时排序，取TOP10
...
```
让结果输出**整齐的表格**，适合多列、多行数据，在后面加上 -t
mysql -u用户名 -p -t

格式化 SQL：在 SQL 语句末尾加上 `\G`（**必须大写**），代替分号 `;`
-- 普通写法（乱换行） 
...
怎么判断慢 SQL？
- `max_time_sec > 1` → 这条 SQL **出现过慢查询**
- `avg_time_sec > 0.5` → 平均就很慢
- `scan_rows_total` 巨大 → 没走索引、全表扫

-- 查询最近的数据
SELECT * FROM performance_schema.events_statements_summary_by_digest order by LAST_SEEN desc limit 2;

-- 查看执行时间超过1秒的SQL
SELECT id, user, db, command, time, state, info
FROM information_schema.processlist
WHERE time > 1 AND command='Query';
```
...
![](./images/show%20full%20processlist.png)
...
### N+1 查询问题
N+1 查询问题指的是，当需要从数据库中获取 N 条记录时，程序先执行一个初始查询获取主对象列表（1 查询），然后对每个对象执行额外的查询（N 查询），导致总共执行了 1 + N 次查询，而实际可以通过合并查询来减少数据库交互次数。
例子：存在 Author(作者)和 Book(书籍)，一个作者可以有多本书。查询次数：1(获取作者) + N(获取每个作者的书) = N+1 次查询
性能影响： 当 N 较小时问题不明显，当 N 很大时(如 1000 个作者)，会产生 1001 次查询，网络往返和查询解析开销显著增加。
解决方案：
```
-- 只需 1 次查询
SELECT a.*, b.*
...
大小写敏感 = 大写、小写字母被当成不同字符；不敏感 = 视为同一个。

```
-- 字段 name 分别存大写、小写
INSERT INTO user (name) VALUES ('Tom'), ('tom');

大小写敏感（例：utf8mb4_bin）
SELECT * FROM user WHERE name = 'Tom'; -- 查到 1 条
SELECT * FROM user WHERE name = 'tom'; -- 查到 1 条

大小写不敏感（默认：utf8mb4_general_ci，ci=case insensitive）
SELECT * FROM user WHERE name = 'Tom'; -- 同时查出 Tom、tom 两条
SELECT * FROM user WHERE name = 'tom'; -- 同时查出 Tom、tom 两条

已有 tom，插入 Tom 提示已经存在，是不敏感
```
