...
@todo  
如何测试一个 http 接口（使用 MySQL）是否存在慢查询的情况
...
例子：存在Author(作者)和Book(书籍)，一个作者可以有多本书。查询次数：1(获取作者) + N(获取每个作者的书) = N+1次查询
性能影响： 当N较小时问题不明显，当N很大时(如1000个作者)，会产生1001次查询，网络往返和查询解析开销显著增加。
解决方案：
```
-- 只需1次查询
SELECT a.*, b.*
