...
4、注:单独部署 core 或者 oms 失败都会同时查看 core 和oms 的日志,如果无法查看启动日志,报错请忽略,请直接前往服务器查看\033[0m"
...
#5、拉取 disconf
core_disconf(){
    echo -e "\033[32m${time} 5、从 disconf 拉取${codeName}-core 的配置文件\033[0m"
    if [[ ${comp} = "fb" ]]; then
...
#5、拉取 disconf
oms_disconf(){
    echo -e "\033[32m${time} 5、从 disconf 拉取${codeName}-oms 的配置文件\033[0m"
    if [[ ${comp} = "fb" ]]; then
...
core_query_log(){
    if [[ ${comp} = "fb" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-core 的启动日志开始,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog ${codeName} core slog\033[0m\n"
    elif [[ ${comp} = "jly" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-core 的启动日志开始,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog jly ${codeName} core slog\033[0m\n"
    fi
    sleep 2s
    cd /app/${comp}-${codeName}/jetty/${codeName}-core/logs && tail -n 200 stdout.log
    sleep 2s
    if [[ ${comp} = "fb" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-core 的启动日志结束,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog ${codeName} core slog\033[0m\n"
    elif [[ ${comp} = "jly" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-core 的启动日志结束,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog jly ${codeName} core slog\033[0m\n"
    fi
}
...
oms_query_log(){
    if [[ ${comp} = "fb" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-oms 的启动日志开始,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog ${codeName} oms slog\033[0m\n"
    elif [[ ${comp} = "jly" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-oms 的启动日志开始,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog jly ${codeName} oms slog\033[0m\n"
    fi
    sleep 38s
    cd /app/${comp}-${codeName}/tomcat/logs && tail -n 200 catalina.out
    sleep 2s
    if [[ ${comp} = "fb" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-oms 的启动日志结束,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog ${codeName} oms slog\033[0m\n"
    elif [[ ${comp} = "jly" ]];then
        echo -e "\n\033[44;37m${time} 11、${codeName}-oms 的启动日志结束,注:日志可能有延迟,以服务器上最新日志为准,查看启动日志：cdlog jly ${codeName} oms slog\033[0m\n"
    fi
}
...
deploy_core(){
    ...
    echo -e "\033[32m${time} 3、停止${comp}-${codeName} core 服务：cd /app/${comp}-${codeName}/jetty/${codeName}-core/bin/ && su ${comp}-${codeName} stop.sh\033[0m"
...
deploy_oms(){
    ...
    echo -e "\033[32m${time} conf 分组：cd /usr/local/apache-tomcat-7.0.106/core-app/${comp}-${codeName} && chown -R ${comp}-${codeName}:coreapp conf\033[0m"
