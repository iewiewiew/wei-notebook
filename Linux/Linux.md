...
本地端口转发（Local Port Forwarding）作用：把远程服务器的端口，映射到你本地电脑的端口，本地访问 localhost:本地端口 就等于访问远程服务。

ssh -L 本地端口:远程服务地址:远程服务端口 远程服务器用户名@远程服务器IP/域名
场景：
- 远程服务器：192.168.1.100
- 远程 MySQL：只允许服务器本地访问（端口 3306）
- 在本地电脑直接连接这个 MySQL：ssh -L 3307:127.0.0.1:3306 root@192.168.1.100
- 本地访问：localhost 3307 即可访问远程 MySQL

注意：
- 127.0.0.1，是【远程机器】上的本地回环地址
- 如果远程数据库绑定的是内网 IP，不是 127.0.0.1，就写成：ssh -L 3307:10.0.0.5:3306 user@远程IP
- 加 -Nf 让 SSH 后台运行，不占用终端
- 关闭后台转发：pkill ssh