...
![Hadoop Cluster](images/hadoop_cluster.png)
...
-p 9876:9876  \
-v `pwd`/mq/data/namesrv/logs:/root/docker/logs \
-v `pwd`/mq/data/namesrv/store:/root/docker/store \
...
cat `pwd`/mq/conf/broker.conf

2、启动容器
docker run -d --restart=always --name rmqbroker \
-p 10911:10911 -p 10909:10909 \
-v `pwd`/mq/data/broker/logs:/root/docker/logs \
-v `pwd`/mq/rocketmq/data/broker/store:/root/docker/store \
-v `pwd`/mq/conf/broker.conf:/opt/rocketmq-4.4.0/conf/broker.conf \
...
-p 9001:8080 \
-v `pwd`/kafka/data:/usr/local/kafka-map/data \
...
```
启动容器 swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/jenkins/jenkins:lts
docker run -d \
--name=jenkins \
--restart=always \
-u root \
...
安装文档：https://docs.gitlab.cn/docs/jh/install/docker/installation  
镜像地址：https://docker.aityp.com/image/docker.io/gitlab/gitlab-ce:latest

```
搜索镜像
docker search gitlab

拉取镜像 GitLab 社区版（CE）镜像：swr.cn-north-4.myhuaweicloud.com/ddn-k8s/docker.io/gitlab/gitlab-ce:latest
docker pull gitlab/gitlab-ce

创建目录
mkdir config logs data

运行容器 端口映射：443(HTTPS), 80(HTTP), 22(SSH)
sudo lsof -ti:8080,8899 | xargs -r sudo kill -9 && echo "进程已终止" || echo "无相关进程"
...
配置文件
vim `pwd`/gitlab/config/gitlab.rb
```
...
用户名: root 密码: EtO1tekKfn6Pn4NjCSdb1xvm58g+FWZHw7NgJkNEkJc=
```
