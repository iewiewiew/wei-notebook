...
cd `dirname $0`
BIN_DIR=`pwd`
deploy_user_name=`whoami`
cd ..
export JAVA_HOME="/usr/java/jdk1.7.0_80"
export PATH="/usr/java/jdk1.7.0_80/bin:$PATH"
export DISCONF_PATH="/app/core/config/disconf.properties"
DEPLOY_DIR=`pwd`
...
SERVER_NAME=`sed '/^app.process.name/!d;s/.*=//' config/dubbo.properties | tr -d '\r'`
...
JAVA_DEBUG_OPTS=" -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,address=1111,server=y,suspend=n "
JAVA_JMX_OPTS=""
if [ "$1" = "jmx" ]; then
    if [ -n "$2" ]; then
...
JAVA_MEM_SIZE_OPTS="-Xmx256m -Xms256m -Xmn128m -XX:PermSize=64m -XX:MaxPermSize=128M -Xss256k"
if [ -n "$BITS" ]; then
...
reportTo "java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $JAVA_PROPERTIES_OPTS $JAVA_GC_OPTS -classpath $CONF_DIR:$LIB_JARS com.tc.framework.dubbo.Main"
#nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $JAVA_PROPERTIES_OPTS $JAVA_GC_OPTS -classpath $CONF_DIR:$LIB_JARS com.tc.framework.dubbo.Main > $STDOUT_FILE 2>&1 &
java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $JAVA_PROPERTIES_OPTS $JAVA_GC_OPTS -classpath $CONF_DIR:$LIB_JARS com.tc.framework.dubbo.Main > $STDOUT_FILE 2>&1
sleep 1
APP_PID=`ps -ef -ww | grep "java" | grep " -DappName=$SERVER_NAME " | awk '{print $2}'`
